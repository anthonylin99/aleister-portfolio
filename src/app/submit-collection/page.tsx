...
export default function SubmitCollectionPage() {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      let visitorId = localStorage.getItem('aleister-visitor-id');
      if (!visitorId) {
        visitorId = crypto.randomUUID();
        localStorage.setItem('aleister-visitor-id', visitorId);
      }
    ...
    return (
      <div className="p-6 lg:p-8 min-h-screen relative flex items-center justify-center">
        ...
        <div className="relative z-10">
          <div className="glass-card p-8 rounded-2xl text-center max-w-md animate-fade-in-up filigree-corners">
            <CheckCircle className="w-16 h-16 text-emerald-400 mx-auto mb-4" />
            <h2 className="text-2xl font-bold text-[var(--gb-parchment)] mb-2 font-cinzel">Collection Submitted!</h2>
            ...
            <div className="flex gap-3 justify-center">
              <Link
                href="/explore"
                className="px-4 py-2 bg-[var(--gb-gold)] text-[var(--gb-parchment)] rounded-lg hover:bg-[var(--gb-gold)]/80 transition-colors"
              >
                Back to Explore
              </Link>
              <button
                ...
                className="px-4 py-2 bg-[var(--gb-azure-deep)] text-[var(--text-secondary)] rounded-lg hover:bg-[var(--gb-azure)] transition-colors"
              >
                Submit Another
              </button>
            </div>
      ...
      <div className="relative z-10">
        ...
        <div className="flex items-center gap-3 mb-8">
          <div className="w-10 h-10 rounded-xl bg-[var(--gb-gold)]/20 flex items-center justify-center">
            <Send className="w-5 h-5 text-[var(--gb-gold)]" />
          </div>
          <div>
            <h1 className="text-3xl font-bold text-[var(--gb-parchment)] font-cinzel">Submit a Collection</h1>
        ...
        <div className="max-w-2xl">
          <div className="glass-card p-6 rounded-2xl space-y-6 filigree-corners">
            {/* Name */}
            <div>
            ...
            <div>
              ...
              <textarea
                ...
                maxLength={500}
                rows={3}
                className="w-full px-3 py-2.5 rounded-xl bg-[var(--gb-azure-deep)]/80 border border-[var(--gb-gold-border)] text-[var(--gb-parchment)] placeholder-[var(--text-subtle)] focus:outline-none focus:border-[var(--gb-gold)] transition-colors resize-none"
              />
              <p className="text-xs text-[var(--text-subtle)] mt-1">{description.length}/500</p>
            </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mb-3">
                ...
                <button
                  ...
                  className="flex items-center gap-1 px-3 py-2 rounded-lg bg-[var(--gb-gold)] text-[var(--gb-parchment)] hover:bg-[var(--gb-gold)]/80 transition-colors text-sm disabled:opacity-50"
                >
              ...
              {tickers.length > 0 && (
                <div className="flex flex-wrap gap-2">
                  {tickers.map((t) => (
                    <span
                      key={t}
                      className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-[var(--gb-azure-deep)] text-sm text-[var(--gb-parchment)] font-mono"
                    >
                      {t}
                      <button
            ...
            <button
              ...
              disabled={submitting}
              className={cn(
                'w-full py-3 rounded-xl font-semibold text-[var(--gb-parchment)] transition-colors',
                submitting
                  ? 'bg-[var(--gb-azure)] cursor-not-allowed'
                  : 'bg-[var(--gb-gold)] hover:bg-[var(--gb-gold)]/80'
              )}
            >
