...
function getIcon(type: string) {
  switch (type) {
    ...
    case 'MEMBER_JOINED':
      return <UserPlus className="w-3.5 h-3.5 text-[var(--gb-gold)]" />;
    case 'PORTFOLIO_CREATED':
      return <Briefcase className="w-3.5 h-3.5 text-cyan-400" />;
    default:
      return <Plus className="w-3.5 h-3.5 text-[var(--text-muted)]" />;
  }
}
...
export function ActivityItem({ event }: ActivityItemProps) {
  return (
    <div className="flex items-start gap-3 py-3">
      <div className="w-7 h-7 rounded-full bg-[var(--gb-azure-deep)] flex items-center justify-center flex-shrink-0 mt-0.5">
        {getIcon(event.type)}
      </div>
      <div className="flex-1 min-w-0">
        <p className="text-sm text-[var(--text-secondary)]">
          <span className="font-medium text-[var(--gb-parchment)]">{event.userName}</span>{' '}
