    ...
    return (
      <div className="glass-card p-6 rounded-2xl">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-lg font-semibold text-[var(--gb-parchment)]">Priority Alerts</h3>
        </div>
        <p className="text-[var(--text-muted)] text-sm">No significant signals detected. Your portfolio is in stable condition.</p>
      </div>
    );
  }
  ...
  return (
    <div className="glass-card p-6 rounded-2xl">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs text-[var(--text-subtle)]">{alerts.length} signal{alerts.length !== 1 ? 's' : ''}</span>
      </div>
      ...
      <div className="space-y-3">
        {alerts.slice(0, 5).map((alert, i) => (
          <Link
            ...
            className={cn(
              'block p-3 rounded-lg border-l-2 hover:bg-[var(--gb-azure-deep)]/30 transition-colors',
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <span className="font-semibold text-[var(--gb-parchment)]">{alert.ticker}</span>
                  <span className={cn(
                    'text-[10px] font-medium px-1.5 py-0.5 rounded',
                    alert.priority === 'high' && 'bg-red-500/20 text-red-400',
                    alert.priority === 'medium' && 'bg-amber-500/20 text-amber-400',
                    alert.priority === 'low' && 'bg-slate-600/50 text-[var(--text-muted)]'
                  )}>
                ...
                {alert.actionHint && (
                  <p className="text-xs text-[#D4AF37] mt-1">{alert.actionHint}</p>
                )}
              </div>
            </div>
          </Link>
